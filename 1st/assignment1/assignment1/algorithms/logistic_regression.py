...
class Logistic(object):
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray, weights: np.ndarray) -> np.ndarray:
        ...
        # TODO: implement me
        yto = np.eye(len(y_train), np.max(y_train)+1, dtype=int)[y_train]
        self.w = weights - (self.lr * (1/N)) * np.dot(2*(self.sigmoid(np.dot(X_train, weights.T)) - yto).T, X_train)
        # TODO: implement me
        return self.w
    ...
    def predict(self, X_test: np.ndarray) -> np.ndarray:
        ...
        Returns:
            predicted labels for the data in X_test; a 1-dimensional array of
                ...
                class.
        """
        preds = self.sigmoid(np.dot(X_test, self.w.T))
